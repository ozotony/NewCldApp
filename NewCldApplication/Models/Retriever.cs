...
namespace WebApplication4.Models
{
    public class Retriever
    {
        ...
        public string ConnectTm2()
        {
            return ConfigurationManager.ConnectionStrings["tmConnectionString2"].ConnectionString;
        }
        ...
        public List<ApplicationOfficer2> ApplicationOff(string startdate, string enddate)
        {
            ...
            SqlConnection connection = new SqlConnection(this.ConnectTm2());
        ...
        public List<ApplicationOfficer3> ApplicationOff2(string startdate, string enddate)
        {
            string str = "";
            int sn = 1;
            ...
            var kk = Convert.ToDateTime(startdate).Date.ToString(format);
            var kk2 = Convert.ToDateTime(enddate).Date.ToString(format);
            SqlCommand command = new SqlCommand("select tm_office.data_status,xname,count(xname) as vcount   from tm_office inner JOIN xadminz_tm ON xadminz_tm.xID=tm_office.xofficer inner JOIN pwallet on pwallet.id =tm_office.pwalletID where  tm_office.reg_date BETWEEN  '" + kk + "' AND '" + kk2 + "'  group by  tm_office.data_status, xname   ", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            List<ApplicationOfficer3> xk = new List<ApplicationOfficer3>();
            while (reader.Read())
            {
                ApplicationOfficer3 pp = new ApplicationOfficer3();
                pp.data_status = reader["data_status"].ToString();
              //  pp.sn = sn;
                pp.xname = reader["xname"].ToString();

                pp.vcount = reader["vcount"].ToString();
        ...
        public String getUpload()
        {
            ...
            SqlCommand command = new SqlCommand("SELECT * FROM Uploads   ", connection)
            {
                CommandTimeout = 0
            };
            connection.Open();
            ...
            while (reader.Read())
            {
                ...
                var kka = reader["TransactionId"].ToString();
                var kk2 = reader["TransactionId2"].ToString();

                if (kka != null)
                {
                    updateTransID2b(kk2, kka);
                }
...

        {
            ...
            else if (criteria == "reg_number")
            {
                if ((status == "4") && ((data_status == "Refused") || (data_status == "Non-registrable")))
                {
                    cmdText = "select top(100) pwallet.rtm,  applicant.xname,mark_info.national_classID 'class',mark_info.xID,mark_info.reg_number 'reg_no',mark_info.product_title,tm_type.type 'tm_type',pwallet.validationID 'oai_no', ISNULL(pwallet.data_status,'None') 'xstat', mark_info.reg_date 'reg_dt',mark_info.log_staff  from mark_info LEFT OUTER JOIN pwallet ON mark_info.log_staff=pwallet.ID   LEFT OUTER JOIN tm_type ON tm_type.xID=mark_info.tm_typeID  LEFT OUTER JOIN applicant ON applicant.log_staff=pwallet.ID   WHERE pwallet.stage='5' AND pwallet.status='" + status + "' AND ((pwallet.data_status='Non-registrable') OR (pwallet.data_status='Refused')) AND mark_info.reg_number like  '%" + fulltext[0] + "%' AND pwallet.reg_date between '" + dateFrom + "' AND '" + dateTo + "'  ORDER BY xID ASC ";
                }
                else if ((status == "1") && ((data_status == "Valid") || (data_status == "Invalid")))
                {
            ...
            while (reader.Read())
            {
                ...
                foreach (TmOffice t2 in lt_tm_office)
                {
        ...
        public Int32 getNewAdminSearchMarkInfoRS444(string status, string data_status, string criteria, List<string> fulltext, string dateFrom, string dateTo, int start, int limit)
...

        {
            ...
            if (criteria == "product_title")
            {
                ...
                str2 = "select  count(1) AS cnt  from pwallet LEFT OUTER JOIN mark_info ON mark_info.log_staff=pwallet.ID   LEFT OUTER JOIN tm_type ON tm_type.xID=mark_info.tm_typeID  LEFT OUTER JOIN applicant ON applicant.log_staff=pwallet.ID   WHERE    pwallet.stage='5' AND CAST(pwallet.status AS INT)>='" + status + "' ";
                // str2 = "select  pwallet.rtm,ROW_NUMBER() OVER(ORDER BY pwallet.id desc ) AS NUMBER,  applicant.xname,mark_info.national_classID 'class',mark_info.xID,mark_info.reg_number 'reg_no',mark_info.product_title,mark_info.logo_pic,tm_type.type 'tm_type',pwallet.validationID 'oai_no', ISNULL(pwallet.data_status,'None') 'xstat', mark_info.reg_date 'reg_dt',mark_info.log_staff  from pwallet LEFT OUTER JOIN mark_info ON mark_info.log_staff=pwallet.ID   LEFT OUTER JOIN tm_type ON tm_type.xID=mark_info.tm_typeID  LEFT OUTER JOIN applicant ON applicant.log_staff=pwallet.ID   WHERE    pwallet.stage='5' AND CAST(pwallet.status AS INT)>='" + status + "'  ) AS TBL WHERE NUMBER BETWEEN((", start, " - 1) * ", limit, " + 1) AND(", start, " * ", limit, ")  " });
                ...
                // str4 = " ORDER BY xID ASC";
              //  string.Concat(new object[] { "SELECT* FROM (" + str2 + str3 + str4 + ") AS TBL WHERE NUMBER BETWEEN((", start, " - 1) * ", limit, " + 1) AND(", start, " * ", limit, ")  " });
                 cmdText = str2 + str3 ;
              //  cmdText = string.Concat(new object[] { "SELECT* FROM (" + str2 + str3 + ") AS TBL WHERE NUMBER BETWEEN((", start, " - 1) * ", limit, " + 1) AND(", start, " * ", limit, ") ORDER BY xID ASC  " });
                //  cmdText = str2 + str4;
            }
            ...
            while (reader.Read())
            {
                num10 = Convert.ToInt32(reader["cnt"]);
            }
            reader.Close();
            connection.Close();
            return num10;
        ...
        public List<XObjs.Office_view> getNewAdminSearchMarkInfoRS333(string status, string data_status, string criteria, List<string> fulltext, string dateFrom, string dateTo, int start, int limit)
...

        {
            ...
            if (start == 1)
            {
                 vcount = getNewAdminSearchMarkInfoRS444(status, data_status, criteria, fulltext, dateFrom, dateTo, start, limit);
            }
            if (criteria == "product_title")
            {
        ...
        public Double SumTrademark()
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'tm' and  substring(fee_details.xreg_date, 1, 4) = YEAR(GETDATE()) and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
                try
                {
                    succ = Convert.ToDouble(reader["cnt"]);
                }
                catch (Exception ex)
                {
                    succ = 0;
        ...
        public Double SumTrademark2(string start_date, string end_date)
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'tm'  AND fee_details.xreg_date BETWEEN '" + start_date + "' AND '" + end_date + "' and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
        ...
        public Double SumPatent()
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'pt' and  substring(fee_details.xreg_date, 1, 4) = YEAR(GETDATE()) and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
        ...
        public Double SumPatent2(string start_date,string end_date)
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'pt' AND fee_details.xreg_date BETWEEN '" + start_date + "' AND '" + end_date + "' and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
        ...
        public Double SumDesign()
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'ds' and  substring(fee_details.xreg_date, 1, 4) = YEAR(GETDATE()) and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
        ...
        public Double SumDesign2(string start_date, string end_date)
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'ds' AND fee_details.xreg_date BETWEEN '" + start_date + "' AND '" + end_date + "'  and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
        ...
        public Double SumAccreditation()
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'ag' and  substring(fee_details.xreg_date, 1, 4) = YEAR(GETDATE()) and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
        ...
        public Double SumAccreditation2(string start_date, string end_date)
        {
            Double succ = 0;
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'ag' AND fee_details.xreg_date BETWEEN '" + start_date + "' AND '" + end_date + "' and twallet.xpay_status='1'", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
        ...
        public void sendUnsuccAlertHtml(XObjs.Registration pp, string total_amt2, XObjs.InterSwitchResponse isr, XObjs.InterSwitchPostFields isr2, XObjs.Twallet bb, XObjs.Applicant cc, List<XObjs.PaymentReciept> lt_pr)
        {
            try {
                ...
                string to = email;
                Email dd1 = new Email();
                dd1.sendMail("PAYX ALERT", to, from, subject, html_msg, "");
                dd1.sendMail("PAYX ALERT", cc.xemail, from, subject, html_msg, "");
    ...
    new MailAddress(px2.Email));
        ...
        public List<GetData2>  getAgentData()
        {
            Applicant applicant = new Applicant();
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select top(10)  sum(CAST(fee_details.init_amt as int)) as cnt ,twallet.xmemberid from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID  INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'tm' and  substring(fee_details.xreg_date, 1, 4) = YEAR(GETDATE()) and twallet.xpay_status='1' group by twallet.xmemberid order by sum(CAST(fee_details.init_amt as int)) desc  ", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            List<GetData2> pp2 = new List<GetData2>();
            while (reader.Read())
            {
                GetData2 pp = new GetData2();
                pp.Amount = Convert.ToDouble(reader["cnt"].ToString());
                var kk = getRegistrationBySubagentRegistrationID(reader["xmemberid"].ToString());
                pp.FirstName = kk.Firstname;
                pp.Surname = kk.Surname;
                pp.AgentId = kk.Sys_ID;
                pp2.Add(pp);
        ...
        public List<GetData2> getAgentDataB(String start_date ,String end_date)
        {
            Applicant applicant = new Applicant();
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select top(10)  sum(CAST(fee_details.init_amt as int)) as cnt ,twallet.xmemberid from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID  INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'tm' AND fee_details.xreg_date BETWEEN '" + start_date + "' AND '" + end_date + "' and twallet.xpay_status='1' group by twallet.xmemberid order by sum(CAST(fee_details.init_amt as int)) desc  ", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            List<GetData2> pp2 = new List<GetData2>();
            while (reader.Read())
        ...
        public List<GetData2> getAgentData3()
        {
            Applicant applicant = new Applicant();
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt ,fee_list.item  from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'tm' and  substring(fee_details.xreg_date, 1, 4) = YEAR(GETDATE()) and twallet.xpay_status='1' group by fee_list.item", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            List<GetData2> pp2 = new List<GetData2>();
            while (reader.Read())
            {
                ...
                pp.Amount = Convert.ToDouble(reader["cnt"].ToString());
             //   var kk = getRegistrationBySubagentRegistrationID(reader["xmemberid"].ToString());
                pp.FirstName = reader["item"].ToString();
               // pp.Surname = kk.Surname;
               // pp.AgentId = kk.Sys_ID;
                pp2.Add(pp);
        ...
        public List<GetData2> getAgentData3B(string start_date , string end_date)
        {
            Applicant applicant = new Applicant();
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt ,fee_list.item  from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'tm' AND fee_details.xreg_date BETWEEN '" + start_date + "' AND '" + end_date + "' and twallet.xpay_status='1' group by fee_list.item", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            List<GetData2> pp2 = new List<GetData2>();
            while (reader.Read())
        ...
        public List<GetData2> getAgentData4()
        {
            Applicant applicant = new Applicant();
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = null;
             command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt ,fee_list.item  from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'pt' and  substring(fee_details.xreg_date, 1, 4) = YEAR(GETDATE()) and twallet.xpay_status='1' group by fee_list.item", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            List<GetData2> pp2 = new List<GetData2>();
            while (reader.Read())
        ...
        public List<GetData2> getAgentData4B(string start_date, string end_date)
        {
            Applicant applicant = new Applicant();
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            SqlCommand command = null;
            command = new SqlCommand("select sum(CAST(fee_details.init_amt as int)) AS cnt ,fee_list.item  from fee_details  INNER JOIN fee_list ON fee_list.xid = fee_details.fee_listID INNER JOIN twallet  ON fee_details.twalletid = twallet.xid where fee_list.xcategory = 'pt' AND fee_details.xreg_date BETWEEN '" + start_date + "' AND '" + end_date + "' and twallet.xpay_status='1' group by fee_list.item", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            List<GetData2> pp2 = new List<GetData2>();
            while (reader.Read())
        ...
        public List<ViewBasket> ViewBasket3(string cat, string agt)
        {
            List<ViewBasket> xlist = new List<ViewBasket>();
            SqlConnection connection = new SqlConnection(this.ConnectXpay());
            //  SqlCommand command = new SqlCommand("SELECT * FROM fee_list WHERE xcategory='" + cat + "' ", connection);
            SqlCommand command = new SqlCommand("select applicant.xname,applicant.address,applicant.xemail,applicant.xmobile,hwallet.*,hwallet.transID+'-'+CAST(hwallet.fee_detailsID AS NVARCHAR)+'-'+CAST(hwallet.xid AS NVARCHAR) AS new_transID,fee_list.item_code,fee_list.xdesc, CONVERT(varchar, CAST(fee_details.init_amt AS money), 1) as init_amt from fee_list  INNER JOIN fee_details ON fee_details.fee_listID = fee_list.xid   INNER JOIN twallet ON  twallet.xid = fee_details.twalletID   INNER JOIN applicant ON  twallet.applicantID = applicant.xid      INNER JOIN hwallet ON hwallet.fee_detailsID = fee_details.xid  where fee_list.xcategory = '" + cat + "'  AND twallet.xmemberID ='" + agt + "'  AND twallet.xpay_status = '1' AND hwallet.used_status = 'Used' ", connection);
        ...
        public List<Stage> getStageByUserIDAcc3(string validationID)
        {
            List<Stage> list = new List<Stage>();
            SqlConnection connection = new SqlConnection(this.Connect3());
            SqlCommand command = new SqlCommand("SELECT * FROM pwallet WHERE validationID='" + validationID + "'   AND stage='5' AND data_status <>'' ", connection);
        ...
        public XObjs.Registration getRegistrationEmail(string email)
        {
            XObjs.Registration x = new XObjs.Registration();
            SqlConnection connection = new SqlConnection(this.ConnectXhome());
            SqlCommand command = new SqlCommand("SELECT * FROM registrations WHERE Email='" + email + "'   ", connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader(CommandBehavior.CloseConnection);
            while (reader.Read())
            {
